...
class Delta_Decoder(nn.Module):
    ...
    def __init__(
        ...
        super(Delta_Decoder, self).__init__()
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end):
        ...
        for _ in range(self.seq_len):
            ...
            embedding_input = curr_pos

            decoder_input = self.spatial_embedding(embedding_input)
            decoder_input = decoder_input.view(1, batch, self.embedding_dim)
            pred_traj_fake_rel.append(curr_pos.view(batch, -1))
            last_pos = curr_pos
            self.state_seq.append(state_tuple)
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, aux_input=None, user_noise=None, seq_len=12):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
...
class LateAttentionGenerator(nn.Module):
    def __init__(
        ...
        noise_type='gaussian', noise_mix_type='ped', pooling_type=None,
        pool_every_timestep=True, dropout=0.0, bottleneck_dim=1024,
        activation='relu', batch_norm=True, neighborhood_size=2.0, grid_size=8, goal_dim=(2,)
    ):
        super(LateAttentionGenerator, self).__init__()
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, aux_input=None, user_noise=None, goal_input=None, seq_len=12):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        - obs_traj_rel: Tensor of shape (obs_len, batch, 2)
        ...
        for t in range(seq_len):
            force_rel_pos, force_state_tuple = self.force_decoder.step_forward(last_pos, last_pos_rel, force_state_tuple, seq_start_end)
...
class LateAttentionFullLineGenerator(nn.Module):
    def __init__(
        ...
        super(LateAttentionFullLineGenerator, self).__init__()

        if pooling_type and pooling_type.lower() == 'none':
            pooling_type = None

        self.spatial_dim=spatial_dim
        self.spatial_dim=True
        self.obs_len = obs_len
        self.pred_len = pred_len
        self.mlp_dim = mlp_dim
        self.encoder_h_dim = encoder_h_dim
        ...
        # attention decision base only on social force
        self.attention_mlp = nn.Linear(decoder_h_dim, 2)
        # nn.init.kaiming_normal_(self.attention_mlp.weight)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, aux_input=None, user_noise=None, goal_input=None, seq_len=12):
        ...
        for t in range(seq_len):
            intention_rel_pos = (goal_input[0] - last_pos_rel)/float(seq_len-t) + last_pos_rel

            intention_pos = intention_rel_pos + obs_traj[0]
            
            force_rel_pos, force_state_tuple = self.force_decoder.step_forward(intention_pos, intention_rel_pos, force_state_tuple, seq_start_end)

            attention_score = self.attention_mlp(force_state_tuple[0].view(-1, self.decoder_h_dim))
